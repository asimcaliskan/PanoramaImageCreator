...
class ImageBlender:
  ...
  def blend(self):
    ...
    for _ in range(self.pyramid_length - 1):
      gauss_applied_image = self.apply_gauss(np.copy(self.left_gauss_pyramid[-1]))
      self.left_gauss_pyramid.append(self.apply_subsampling(gauss_applied_image))
    
    #right
    self.right_gauss_pyramid.append(self.right_image)
    for _ in range(self.pyramid_length - 1):
      gauss_applied_image = self.apply_gauss(np.copy(self.right_gauss_pyramid[-1]))
      self.right_gauss_pyramid.append(self.apply_subsampling(gauss_applied_image))
    #
    
    #create left and right upsampled images list
    #left
    for img in range(1, len(self.left_gauss_pyramid)):
      upsampled_image = self.apply_upsampling(np.copy(self.left_gauss_pyramid[img]))
      self.left_upsampled_images.append(upsampled_image)
    
    #right
    for img in range(1, len(self.right_gauss_pyramid)):
      upsampled_image = self.apply_upsampling(np.copy(self.right_gauss_pyramid[img]))
    ...
    for img in range(len(self.left_upsampled_images) - 1, -1, -1):
      if len(self.blended_images) == 0:
        ...
        self.blended_images.append(self.apply_upsampling(np.add(concatenated_upsamples, self.combined_laplacian_pyramid[img])))
      else:
