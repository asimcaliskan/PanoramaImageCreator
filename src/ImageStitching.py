...
def main():
    blended_image = None
    for img in range(len(input_images) - 1):
        if img == 0:
            left_image = cv2.imread(input_images[img])
            right_image = cv2.imread(input_images[img + 1])
            blended_image = ImageStitching().blending(left_image, right_image)
        else:
            right_image = cv2.imread(input_images[img + 1])
